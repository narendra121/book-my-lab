...
func WriteAppResponse(msg string, appErr error, obj any) *dto.APIResponse {
	apiRsp := &dto.APIResponse{
		Message: msg,
	}
	if appErr == nil {
		apiRsp.Status = constants.Success
		if obj != nil {
			bytesData, _ := json.Marshal(obj)
			apiRsp.Data = string(bytesData)
		}
	} else {
		apiRsp.Error = appErr.Error()
	}
	return apiRsp
}
